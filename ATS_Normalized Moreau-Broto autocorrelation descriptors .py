# -*- coding: utf-8 -*-
"""
Created on Tue May 10 06:37:54 2022
...
for j in c[1:len(c)]:
    ...
    for i in range(len(data)):
        ...
        for m in range(len(se[i])-d):
           ...
           n=(n+a1*a2)/(len(se[i])-d)
        data.loc[:,j][i]=n
data=data.set_index('id')
data.to_csv(Out+'/'+File.split('.')[0]+'_ATS_Normalized Moreau-Broto autocorrelation descriptors.csv')