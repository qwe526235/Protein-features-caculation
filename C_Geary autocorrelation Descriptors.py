# -*- coding: utf-8 -*-
"""
Created on Tue May 10 09:49:54 2022
...
for j in c[1:len(c)]:
    ...
    for i in range(len(data)):
        ...
        data.loc[:,j][i]=(n/(2*(len(se[i])-d)))/(t/len(se[i])-1)
data=data.set_index('id')
data.to_csv(Out+'/'+File.split('.')[0]+'_C_Geary autocorrelation Descriptors.csv')